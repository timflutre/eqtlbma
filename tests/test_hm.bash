...
function help () {
    ...
    msg+="      --p2b\tabsolute path to the 'eqtlbma_bf' binary\n"
    msg+="      --p2h\tabsolute path to the 'eqtlbma_hm' binary\n"
...
function parseArgs () {
    TEMP=`getopt -o hVv: -l help,version,verbose:,p2b:,p2h:,p2R:,noclean \
        -n "$0" -- "$@"`
    ...
    while true; do
        case "$1" in
            ...
            --p2b) pathToBf=$2; shift 2;;
            --p2h) pathToHm=$2; shift 2;;
    ...
    if [[ ! -f $pathToHm ]]; then
	echo "ERROR: can't find path to 'eqtlbma_hm' -> '${pathToBf}'"
	exit 1
    fi
