...
# Aim: launch a basic functional test for eqtlbma_bf with some SNPs with
#      low MAF
...
function help () {
  msg="\`${0##*/}' launches a basic functional test for eqtlbma_bf with some SNPs with low MAF.\n"
  msg+="\n"
  msg+="Usage: ${0##*/} [OPTIONS] ...\n"
  msg+="\n"
  msg+="Options:\n"
...
function simul_data_and_calc_exp_res () {
  ...
  ${pathToRscript} --verbose $(($verbose - 1)) --dir $(pwd) \
    --lmaf >& stdout_simul_exp
}
...
function calc_obs_res () {
  ...
  $pathToBf --geno list_genotypes.txt --scoord snp_coords.bed.gz \
	  ...
	  -v $(($verbose - 1)) --maf 0.05 >& stdout_bf
}
