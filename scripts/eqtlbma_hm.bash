...
# Aim: launch the hierarchical model from output files of 'eqtlbma_bf'
# Author: Timothee Flutre
# Not copyrighted -- provided to the public domain

function help () {
    msg="\`${0##*/}' launches the hierarchical model from output files of eqtlbma_bf.\n"
    msg+="\n"
    msg+="Usage: ${0##*/} [OPTIONS] ...\n"
    msg+="\n"
    msg+="Options:\n"
    ...
    msg+="      --p2b\tpath to the binary 'eqtlbma_hm'\n"
    msg+="      --inp\tgzipped files as a glob pattern\n"
    msg+="\t\tusually 'X_l10abfs_raw.txt.gz' files generated by 'eqtlbma_bf'\n"
    msg+="      --inf\tinput file already formatted for '${0##*/}'\n"
    msg+="\t\tuse --inp or --inf, not both\n"
    msg+="      --nbC\tnumber of active configurations (e.g. 7 if 3 subgroups)\n"
    ...
    msg+="      --keepin\tkeep input file generated for 'hm'\n"
    msg+="      --thread\tnumber of threads (default=1)\n"
    msg+="\n"
    msg+="Examples:\n"
    msg+="  ${0##*/} --p2b ~/bin/eqtlbma_hm --inp \"out_eqtlbma_[0-9][0-9][0-9]_l10abfs_raw.txt.gz\" --nbC 7 --nbG 10 --outp out_hm\n"
    msg+="  ${0##*/} --p2b ~/bin/eqtlbma_hm --inp \"out_eqtlbma_[0-9][0-9][0-9]_l10abfs_raw.txt.gz\" --nbC 3 --nbG 10 --outp out_hm_1-3 --reconf \"^1$|^3$|^1-3$\""
    echo -e "$msg"
}
...
function parseArgs () {
    TEMP=`getopt -o hVv: -l help,version,verbose:,p2b:,inp:,inf:,nbC:,nbG:,outp:,reconf:,skip-ci,pi0:,keepin,thread: \
	-n "$0" -- "$@"`
...
for inFile in $inFiles; do
    ...
    if [ -z "${reConf}" ]; then
    	zcat $inFile \
    	    | awk -v g=${nbGrids} 'NR>1 {if(match($3,"gen")==0){printf "%s %s %s", $1, $2, $3; for(i=4;i<=4+g-1;++i){printf " %s", $i}; printf "\n"}}' \
    	    >> $tmpFile
    else
    	zcat $inFile \
    	    | awk -v g=${nbGrids} -v r=$reConf 'NR>1 {if(match($3,"gen")==0 && match($3,r)!=0){printf "%s %s %s", $1, $2, $3; for(i=4;i<=4+g-1;++i){printf " %s", $i}; printf "\n"}}' \
    	    >> $tmpFile
    fi
done
let "nbPairs = $(wc -l < $tmpFile) / $nbConfigs"
if [ $verbose -gt "0" ]; then
