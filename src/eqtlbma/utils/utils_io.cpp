 ...
 */
  ...
  split(
    ...
    vector<string> & tokens)
  {
    return split((char *) s.c_str(), delim, tokens);
  }
 ...
 */
  ...
  split(
    ...
    const size_t & idx)
  {
    vector<string> tokens = split((char *) s.c_str(), delim);
    if(tokens.size() < idx){
...
*/
  ...
  {
    string pathToFile = "/proc/sys/vm/drop_caches";
    if(! doesFileExist(pathToFile)){ // in other OS than Linux
      cerr << "WARNING: " << pathToFile << " doesn't exist,"
	   << " can't track memory usage" << endl << flush;
    }
